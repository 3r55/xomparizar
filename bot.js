...
**Now playing:** ${serverQueue.songs[0].title}
  ...
  function play(guild, song) {
    ...
    serverQueue.textChannel.send(`🎶 Start playing: **${song.title}**`); //${! .₳Ⱨ₥ɆĐ ♡}#0561
  }
...
const ownerID = ["378611609804996609"]; // ايدي ادارة البوت او صاحب البوت ..
...
client.on('ready', () => {
    console.log(`Created By: Black jack`);
...
function play(guild, song) {
  ...
  if(serverQueue.repeating) {
	console.log('Repeating');
     ...
     msg.channel.send(':watch: Loading... [`' + args + '`]').then(p => {
      setTimeout(() => {//:watch: Loading... [let]
       p.edit(':notes: Added **' + song.title + '** (`' + song.duration + '`) to begin playing.');
})
  })
  }
